...
const Navbar = () => {
  ...
  // Scroll-driven hide/show animation
  useEffect(() => {
    let lastY = typeof window !== 'undefined' ? window.scrollY : 0;
    let animating = false;
    let hidden = false;

    const collectAllVisibleChars = () => {
      if (!splitReadyRef.current) return [];
      const all = [];
      menuLinkContainersRef.current.forEach((link) => {
        ...
        const visibleChars = visibleCopy.querySelectorAll('.char');
        visibleChars.forEach((c) => all.push(c));
      });
      return all;
    };

    const hideNavbar = () => {
      if (animating || hidden) return;
      animating = true;
      const tl = gsap.timeline({ onComplete: () => { animating = false; hidden = true; } });
      const allVisibleChars = collectAllVisibleChars();
      // 1) Todas las letras desaparecen hacia arriba, todas a la vez (sin stagger)
      if (allVisibleChars.length) {
        tl.to(allVisibleChars, { y: '-110%', stagger: 0.01, duration: 0.25, ease: 'expo.inOut' });
      }
      // Asegurar que cualquier copia animada visible por hover también quede fuera
      const allAnimatedChars = [];
      menuLinkContainersRef.current.forEach((link) => {
        if (!link) return;
        const spans = link.querySelectorAll('a span');
        const animatedCopy = spans[1];
        if (!animatedCopy) return;
        animatedCopy.querySelectorAll('.char').forEach((c) => allAnimatedChars.push(c));
      });
      if (allAnimatedChars.length) {
        tl.to(allAnimatedChars, { y: '-110%', duration: 0.001, ease: 'none', overwrite: 'auto' }, '<');
      }
      // 1.b) Subir el nav-p como bloque al mismo tiempo que las letras
      if (navPRef.current) {
        tl.to(navPRef.current, { y: '-110%', duration: 0.25, ease: 'expo.inOut' }, '<');
      }
      // 2) Luego la barra se recoge hacia arriba
      tl.to(navbarRef.current, { scaleY: 0, transformOrigin: 'top center', duration: 1, ease: 'power4.in' });
    };

    const showNavbar = () => {
      // Solo abrir cuando la vista está en el tope de la página
      const atTop = typeof window !== 'undefined' ? window.scrollY <= 4 : true;
      if (!atTop) return;
      if (animating || !hidden) return;
      animating = true;
      const tl = gsap.timeline({ onComplete: () => { animating = false; hidden = false; } });
      // Recopilar chars de todas las palabras
      ...
      menuLinkContainersRef.current.forEach((link) => {
        ...
        const visibleCopy = spans[0];
        const animatedCopy = spans[1];
        if (visibleCopy) visibleCopy.querySelectorAll('.char').forEach((c) => allVisibleChars.push(c));
        if (animatedCopy) animatedCopy.querySelectorAll('.char').forEach((c) => allAnimatedChars.push(c));
      });

      // Asegurar que la copia animada quede debajo (110%) para que el hover siga funcionando
      if (allAnimatedChars.length) tl.set(allAnimatedChars, { y: '110%' });

      // 1) Reabrir barra
      tl.to(navbarRef.current, { scaleY: 1, transformOrigin: 'top center', duration: 1, ease: 'power4.out' });
      // 1.b) Bajar nav-p completo con el mismo efecto (desde 110% a 0)
      if (navPRef.current) {
        // No hacer reset: animar desde su estado actual (probablemente -110%) a 0%
        tl.to(navPRef.current, { y: '0%', duration: 0.3, ease: 'expo.inOut' }, '>-0.05');
      }
      // 2) Traer visibles a su lugar (todas a la vez, sin stagger)
      if (allVisibleChars.length) tl.to(allVisibleChars, { y: '0%', duration: 0.3, ease: 'expo.inOut' }, '>-0.05');
    };
